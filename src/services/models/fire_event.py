...
def parse_fire_event(
    ...
    return FireEvent(
        ...
        ID=row["ID"],
        Address=row["Address"],
        Incident_Date=try_dateformats(row["Incident Date"], EFFECTIVE_DATE_FORMAT),
        Alarm_DtTm=try_dateformats(row["Alarm DtTm"], EFFECTIVE_DATE_FORMAT),
        Arrival_DtTm=try_dateformats(row["Arrival DtTm"], EFFECTIVE_DATE_FORMAT),
        Close_DtTm=try_dateformats(row["Close DtTm"], EFFECTIVE_DATE_FORMAT),
        ...
        Estimated_Property_Loss=(
            row["Estimated Property Loss"] if row["Estimated Property Loss"] else None
        ),
        Estimated_Contents_Loss=(
            row["Estimated Contents Loss"] if row["Estimated Contents Loss"] else None
        ),
        ...
        Ignition_Factor_Primary=(
            row["Ignition Factor Primary"] if row["Ignition Factor Primary"] else None
        ),
        ...
        Structure_Status=row["Structure Status"] if row["Structure Status"] else None,
        ...
        Detector_Failure_Reason=(
            row["Detector Failure Reason"] if row["Detector Failure Reason"] else None
        ),
...
def fire_event_to_key(row: FireEvent) -> str:
    """
    Generate a unique key for a FireEvent object based on its ID and Incident Date.

    :param row: A FireEvent object.
    :return: A string representing the unique key for the FireEvent.
    """
    return f"{row.ID}_{row.Incident_Date.strftime('%Y%m%d')}" if row.Incident_Date else row.ID